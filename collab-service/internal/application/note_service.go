...
func (s *NoteService) Create(c *gin.Context, note *entity.Note) error {
	...
	if note.FolderID != uuid.Nil {
		...
		if !folderAccessLevel.GreaterThan(entity.AccessLevelWrite) {
			return NewForbiddenError("You do not have write permission for this folder")
		}
	}

	return s.repo.Create(c.Request.Context(), note, userID)
}
...
func (s *NoteService) ShareNote(c *gin.Context, noteID, userID uuid.UUID, accessLevel entity.AccessLevel) error {
	...
	return s.repo.ShareNote(c.Request.Context(), noteID, userID, accessLevel)
}
...
func (s *NoteService) RevokeAccess(c *gin.Context, noteID, userID uuid.UUID) error {
	...
	return s.repo.RevokeAccess(c.Request.Context(), noteID, userID)
}
...
func (s *NoteService) Update(c *gin.Context, note *entity.Note) error {
	...
	return s.repo.Update(c.Request.Context(), note)
}
...
func (s *NoteService) Delete(c *gin.Context, id uuid.UUID) error {
	...
	return s.repo.Delete(c.Request.Context(), id)
}
