...
func (r *TeamRepositoryImpl) Create(ctx context.Context, team *entity.Team) (*entity.Team, error) {
	...
	model := model.TeamModelFromDomain(team)
...
func (r *TeamRepositoryImpl) ExistsByID(ctx context.Context, id uuid.UUID) (bool, error) {
	var count int64
	if err := r.db.WithContext(ctx).Model(&model.TeamModel{}).Where("id = ?", id).Count(&count).Error; err != nil {
		return false, err
	}
	return count > 0, nil
}
...
func (r *TeamRepositoryImpl) RemoveMember(ctx context.Context, teamID uuid.UUID, memberID uuid.UUID) error {
	return r.db.WithContext(ctx).Where("team_id = ? AND user_id = ?", teamID, memberID).Delete(&model.RosterModel{}).Error
}
...
func (r *TeamRepositoryImpl) Update(ctx context.Context, team *entity.Team) (*entity.Team, error) {
	teamModel := model.TeamModelFromDomain(team)

	if err := r.db.WithContext(ctx).Model(&teamModel).Updates(map[string]interface{}{
...
func (r *TeamRepositoryImpl) Delete(ctx context.Context, id uuid.UUID) error {
	...
	if err := tx.Delete(&model.RosterModel{}, "team_id = ?", id).Error; err != nil {
		tx.Rollback()
		return err
	}

	// Then delete the team
	if err := tx.Delete(&model.TeamModel{}, "id = ?", id).Error; err != nil {
		tx.Rollback()
		return err
	}
